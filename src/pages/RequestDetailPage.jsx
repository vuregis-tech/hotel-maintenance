...
import { api, imgUrl, schedToISO, activeWorkOrder } from '../lib/api'
...
export default function RequestDetailPage() {
  ...
  async function handleCancel() {
    ...
    try { const u = await api.cancelJob(job.id); setJob(u); toast.success(t('workOrder.toast.cancelSuccess')) }
    catch (err) { toast.error(err.message) }
  }
  ...
  if (!job) return <div className="p-8 text-center text-gray-400 text-sm">{t('workOrder.notFound')}</div>

  // ใช้ WO ตัวล่าสุดที่ยัง active (ไม่ใช่ตัวแรกที่เจอ) — ให้ตรงกับ JobDrawer
  const wo = activeWorkOrder(job) || job.work_orders?.[0]
  ...
  const canExtFollowup = isSuperAdmin && job.status === 'external_tech'
  const canEdit = (
    (job.status === 'pending') ||
    ...
    <div className="max-w-3xl mx-auto space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {job.guest_inhouse && <span className="text-xs px-2 py-0.5 bg-purple-100 text-purple-700 rounded-full">{t('workOrder.guestInHouseTag')}</span>}
            <StatusBadge status={job.status} />
          </div>
          <p className="text-sm text-gray-600 font-medium mt-1">
            {job.reported_at ? format(new Date(job.reported_at), t('common.datetimeFormat'), { locale: dateLocale }) : ''}
          </p>
        </div>
      </div>
      ...
      <Section title={t('workOrder.infoSection')}>
        <div className="space-y-2.5">
          <InfoRow label={t('workOrder.reporterLabel')} value={`${job.reporter?.full_name} (${job.reporter?.position})`} />
        ...
        <Section title={t('workOrder.repairSection')}>
          <div className="space-y-2.5">
            ...
            {wo.co_assignments?.length > 0 && (
              <div className="flex gap-3">
                <span className="text-sm text-gray-500 w-32 flex-shrink-0">{t('workOrder.coTechLabel')}</span>
            ...
            {parsedMaterials.length > 0 && (
              <div className="flex gap-3">
                <span className="text-sm text-gray-500 w-32 flex-shrink-0">{t('workOrder.materialsLabel')}</span>
                <div className="flex-1">
      ...
      {latestInspection && (
        <Section title={t('workOrder.inspectSection')}>
          <div className="space-y-2.5">
            ...
            <div className="flex gap-3">
              <span className="text-sm text-gray-500 w-32 flex-shrink-0">{t('workOrder.inspectResult')}</span>
      ...
      {/* Assign */}
      {modal === 'assign' && (
        <Modal title={t('workOrder.assignModal')} onClose={() => setModal(null)}>
      ...
      {/* Accept */}
      {modal === 'accept' && (
        <Modal title={t('workOrder.acceptModal')} onClose={() => setModal(null)}>
      ...
      {/* Reassign */}
      {modal === 'reassign' && (
        <Modal title={t('workOrder.reassignModal')} onClose={() => setModal(null)}>
          <TechCheckList {...techListProps} />
          <div className="flex gap-2">
            <button onClick={() => setModal(null)} className="flex-1 border border-gray-300 text-gray-700 py-2 rounded-lg text-sm">{t('common.cancel')}</button>
            <button onClick={() => handleAssign('reassign')} disabled={acting}
              className="flex-1 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white py-2 rounded-lg text-sm font-medium">
              {acting ? t('common.saving') : t('workOrder.reassign')}
            </button>
          </div>
        </Modal>
      )}
      ...
      {/* Co-assign */}
      {modal === 'coassign' && (
        <Modal title={t('workOrder.coAssignModal')} onClose={() => setModal(null)}>
          {selectedTechs.length > 0 && (
      ...
      {/* Complete */}
      {modal === 'complete' && (
        <Modal title={t('workOrder.complete')} onClose={() => setModal(null)}>
      ...
      {/* Inspect */}
      {modal === 'inspect' && (
        <Modal title={t('workOrder.inspectModal')} onClose={() => setModal(null)}>
          <div className="space-y-3">
            <div className="flex gap-3">
      ...
      {/* Recall */}
      {modal === 'recall' && (
        <Modal title={t('workOrder.recallModal')} onClose={() => setModal(null)}>
          <div className="space-y-4">
            {wo?.technician && (
              <div className="flex items-start gap-3 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                ...
                <div>
                  <p className="text-sm font-medium text-amber-800">
                    {t('workOrder.recallFrom')} <strong>{wo.technician.full_name}</strong>
                  </p>
            ...
            <div>
              <label className="text-sm font-medium text-gray-700 mb-2 block">{t('workOrder.afterRecall')}</label>
              <div className="space-y-2">
                <label className="flex items-center gap-3 p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50">
                  ...
                  <div>
                    <p className="text-sm font-medium text-gray-800">{t('workOrder.recallToPending')}</p>
                    <p className="text-xs text-gray-500">{t('workOrder.recallToPendingDesc')}</p>
                  </div>
                </label>
                <label className="flex items-center gap-3 p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50">
                  <input type="radio" name="recall_action" value="reassign"
                    ...
                    className="text-blue-600" />
                  <div>
                    <p className="text-sm font-medium text-gray-800">{t('workOrder.recallReassign')}</p>
                    <p className="text-xs text-gray-500">{t('workOrder.recallReassignDesc')}</p>
                  </div>
                </label>
              </div>
            </div>
                ...
                {(() => {
                  ...
                  return (
                    <div className="max-h-48 overflow-y-auto border border-gray-200 rounded-lg divide-y divide-gray-100">
                        ...
                        <>
                          ...
                          {onDutyList.map(tech => (
                            <label key={tech.id} className="flex items-center gap-3 p-2 cursor-pointer hover:bg-gray-50">
                              <input type="radio" name="recall_new_tech" value={tech.id} className="w-4 h-4 text-blue-600" />
                              <span className="text-sm font-medium text-gray-900">{tech.full_name}</span>
                      ...
                      {offDutyList.length > 0 && (
                        <>
                          {onDutyList.length > 0 && <div className="px-3 py-1.5 bg-gray-50 text-xs font-semibold text-gray-500 sticky top-0">{t('workOrder.allTechsSection')}</div>}
                          {offDutyList.map(tech => (
                            <label key={tech.id} className="flex items-center gap-3 p-2 cursor-pointer hover:bg-gray-50">
                              <input type="radio" name="recall_new_tech" value={tech.id} className="w-4 h-4 text-blue-600" />
                              <span className="text-sm font-medium text-gray-900">{tech.full_name}</span>
                              <span className="text-xs text-gray-500">({tech.department})</span>
      ...
      {/* Reject */}
      {modal === 'extFollowup' && (
        <Modal title={extForm.mark_done ? t('workOrder.ext.doneModal') : t('workOrder.ext.followupModal')} onClose={() => setModal(null)}>
          <div className="space-y-3">
      ...
      {modal === 'reject' && (
        <Modal title={t('workOrder.rejectModal')} onClose={() => setModal(null)}>
          <div className="space-y-3">
      ...
      {/* Transfer */}
      {modal === 'transfer' && (
        <Modal title={t('workOrder.transferModal')} onClose={() => setModal(null)}>
          <div className="space-y-3">
      ...
      {/* Edit */}
      {modal === 'edit' && (
        <Modal title={t('workOrder.editModal')} onClose={() => setModal(null)}>
          <div className="space-y-4">
            <div>
      ...
      {/* Location History */}
      {modal === 'history' && (
        <Modal title={t('workOrder.locationHistoryModal')} onClose={() => setModal(null)}>
